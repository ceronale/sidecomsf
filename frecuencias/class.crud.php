...
class crud
{
	...
	public function dataview_frecuencia()
	{
		$user = $_SESSION['user'];
		$query = "SELECT * FROM tipo_frecuencia_salarial";
	...
	public function crear_frecuencia()
	{
		try {
			...
			$stmt_check = $this->conn->prepare("SELECT COUNT(*) FROM tipo_frecuencia_salarial WHERE  nombre = :nombre");
			...
			$stmt = $this->conn->prepare("INSERT INTO tipo_frecuencia_salarial(nombre,status,creacion,modificado) 
	...
	public function editar_frecuencia()
	{
		try {
			...
			$stmt_check = $this->conn->prepare("SELECT COUNT(*) FROM tipo_frecuencia_salarial WHERE  LOWER(nombre) = :nombre AND id != :id");
			...
			$stmt = $this->conn->prepare("UPDATE tipo_frecuencia_salarial SET 
            nombre=:nombre, 
            status=:status, 
	...
	public function eliminar_frecuencia($id)
	{
		$stmt = $this->conn->prepare("DELETE FROM tipo_frecuencia_salarial WHERE id=:id");
		$stmt->bindparam(":id", $id);
		$stmt->execute();
		return true;
	}
